...
void ExpertDemo::init()
{
    try{
        ...
        r1_controller_time_msg.data = jpc_travel_time_;
        r1_controller_time_pub_.publish(r1_controller_time_msg);
        r2_controller_time_msg.data = jpc_travel_time_;
        r2_controller_time_pub_.publish(r2_controller_time_msg);
    }
...
void ExpertDemo::pick(RobotID robot_id, const std::string& brick_name, int press_side, int press_offset)
{
    ...
    prepare_T(2, 3) += 0.1;  // Move up 10cm
    Eigen::Matrix4d perturb = random_perturbation(0.03, 15.0); // add some random perturbation
...
int main(int argc, char **argv)
{
    ...
    int press_side = 1; // 1/4: length side, 2/3: width side
    int press_offset = 1; // 0 - 2 for 2x4 brick

    for (int episode = 0; episode < max_episodes; ++episode) {
        ROS_INFO("Starting episode %d", episode + 1);
        expert_demo.homing(RobotID::Robot1);
        expert_demo.reset_brick(brick_name, 0, 8, 30, 1); // Reset brick position in Gazebo
